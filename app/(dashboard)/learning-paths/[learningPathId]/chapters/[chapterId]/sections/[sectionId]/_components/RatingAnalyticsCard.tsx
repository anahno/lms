// فایل: .../_components/RatingAnalyticsCard.tsx
"use client";

import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Progress } from "@/components/ui/progress"; // +++ کامپوننت Progress را وارد می‌کنیم +++

interface RatingAnalyticsCardProps {
  ratings: (number | null)[];
}

export const RatingAnalyticsCard = ({ ratings }: RatingAnalyticsCardProps) => {
  ...
  if (totalRatings === 0) {
    return (
        <Card className="mt-6 bg-slate-100 border-slate-200">
            <CardHeader>
                <CardTitle className="text-xl">تحلیل امتیازات</CardTitle>
            </CardHeader>
            <CardContent>
                <p className="text-sm text-muted-foreground text-center py-8">هنوز هیچ امتیازی برای این بخش ثبت نشده است.</p>
            </CardContent>
        </Card>
    );
  }
