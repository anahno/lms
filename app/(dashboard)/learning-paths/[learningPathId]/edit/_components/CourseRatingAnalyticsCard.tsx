// فایل: .../_components/CourseRatingAnalyticsCard.tsx
"use client";

import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Progress } from "@/components/ui/progress";

interface CourseRatingAnalyticsCardProps {
  ratings: (number | null)[];
}

export const CourseRatingAnalyticsCard = ({ ratings }: CourseRatingAnalyticsCardProps) => {
  ...
  if (totalRatings === 0) {
    return (
        <Card className="mt-6 bg-slate-100 border-slate-200">
            <CardHeader>
                <CardTitle className="text-xl">تحلیل امتیازات دوره</CardTitle>
            </CardHeader>
            <CardContent>
                <p className="text-sm text-muted-foreground text-center py-8">هنوز هیچ امتیازی در این دوره ثبت نشده است.</p>
            </CardContent>
        </Card>
    );
  }
